...
it("ExerciceInfo : s’affiche avec toutes les infos", () => {
  ...
  data.aides.forEach((element) => {
    expect(
      container.querySelector("[data-testid='aides-test']").textContent
    ).toContain(element);
  });
  data.auteurs.forEach((element) => {
    expect(
      container.querySelector("[data-testid='auteurs-test']").textContent
    ).toContain(element);
  });
  expect(
    container.querySelector("[data-testid='enonce-test']").textContent
  ).toContain(data.enonce);
});
